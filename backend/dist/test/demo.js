...
function main() {
    ...
    const engine = new game_engine_service_1.GameEngineService(deckService, provablyFair, multiplierService, comparator);
    line();
    ...
    let state = engine.startGame({
        userId: 'user-123',
        betAmount: 10,
        config: { aceMode: game_config_interface_1.AceMode.HIGH, equalRule: game_config_interface_1.EqualRule.PUSH },
    });
    ...
    while (state.status === 'ACTIVE' && turn < 10) {
        ...
        const prediction = currentRank <= 7 ? game_config_interface_1.PredictionType.HIGHER : game_config_interface_1.PredictionType.LOWER;
        const outcome = engine.submitGuess(state, prediction);
        state = outcome.state;
        console.log(`Turn ${turn}: current=${currentCard.code} predicted=${prediction} revealed=${outcome.revealedCardCode} ` +
