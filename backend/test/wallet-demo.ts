...
import { InsufficientFundsException, DuplicateTransactionReferenceException } from '../src/wallet/exceptions/wallet.exceptions';
...
async function main() {
  ...
  const wallet = new WalletService(repo as any);
  ...
  try {
    ...
    console.log(`Correctly rejected: ${err instanceof InsufficientFundsException} (${(err as Error).message})`);
  }
